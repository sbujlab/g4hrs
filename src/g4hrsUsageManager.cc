/////////////////////////////////////////////////////////////////////////////
// This is the implement of calss g4hrsUsageManager
...
g4hrsUsageManager* g4hrsUsageManager::fUsageManager=0;
g4hrsUsageManager* g4hrsUsageManager::GetUsageManager()
{ 
	if(!fUsageManager) 
	{
		std::cout<<"No instance of g4hrsUsageManager found! "
			<<"Use g4hrsUsageManager::g4hrsUsageManager(filename,int argc, char **argv) to build one in your main()!"<<std::endl;
	}
	return fUsageManager; 
}


g4hrsUsageManager::g4hrsUsageManager(const char *inifile)
{
#ifdef USAGEMANAGER_DEBUG
	if(Global_Debug_Level < (int)USAGEMANAGER_DEBUG) 
	{
		SetGlobalDebugLevel("g4hrsUsageManager::g4hrsUsageManager",(int)USAGEMANAGER_DEBUG);
	}
#endif

	if(fUsageManager)
	{ 
		cout<<"Error: g4hrsUsageManager constructed twice.\n"
			<<"Use g4hrsUsageManager::Getg4hrsUsageManager() to get the static pointer g4hrsUsageManager::fUsageManager\n";
		exit(-1);
	}
	fUsageManager=this;
...
g4hrsUsageManager::~g4hrsUsageManager()
{
	MapBr_i.clear();
	MapBr_f.clear();
	MapBr_s.clear();
	...
	//cout << "deleting g4hrsUsageManager ......" << endl;
	fUsageManager = 0;
}

//////////////////////////////////////////////////////////////////////
//if unit==0, write into file only, otherwise write to both the file and the screen
void g4hrsUsageManager::WriteLog(const char *str,const char *filename, int unit)
{
    return;
}

//if unit==0, write into file only, otherwise write to both the file and the screen
void g4hrsUsageManager::WriteLog(const char *str)
{
...
void g4hrsUsageManager::ReplaceAll(string& strT,const char *oldsubstr, const char *newsubstr)
{
...
void g4hrsUsageManager::ReplaceAll(char *targetstr,const char *oldsubstr, const char *newsubstr)
{
...
void g4hrsUsageManager::CreateFileName(const char *infilename,char outfilename[], int start, bool overwritten)
{
	FILE *f;
...
void g4hrsUsageManager::CreateFileName(char *instr,char *outstr,int outtype)
{
...
void  g4hrsUsageManager::Trim(std::string &astr)
{
...
void g4hrsUsageManager::Trim(char *astr)
{
...
std::string g4hrsUsageManager::TrimString(std::string &astr)
{
	Trim(astr);
	return astr;
}

std::string g4hrsUsageManager::TrimString(char *astr)
{
...
std::string g4hrsUsageManager::ReadOneValidLine(ifstream &ifs)
{
	char line[1024];
...
size_t g4hrsUsageManager::SplitList(const string str, string list[],size_t allownum)
{
	size_t i=0;
	char *cstr=0, *p=0;	
...
void g4hrsUsageManager::PrintBranch(branch_t &br,bool quit)
{
...
void g4hrsUsageManager::BuildMap()
{
...
void g4hrsUsageManager::PrintMap(ostream &pOut)
{
...
void g4hrsUsageManager::PrintParamMap(ostream &pOut)
{
...
void g4hrsUsageManager::PrintOpt(ostream &pOut)
{
...
void g4hrsUsageManager::PrintUsage(bool bExitAfterPrint)
{		
	cout<<"*********************************MENU**START******************************"<<endl;
	cout<<"This is the usage generated from class g4hrsUsageManager. \n"
...
void g4hrsUsageManager::ReadIni(const char *inifile)
{
	ifstream ifs;
	...
	{
	  //sprintf(strLog,"g4hrsUsageManager::ReadIni() is trying to read file \"%s\" ",inifile);
	  WriteLog(strLog);
	}
...
void g4hrsUsageManager::ProcessArgv(int argc, char **argv)
{  
#ifdef USAGEMANAGER_DEBUG
	if(Global_Debug_Level>=3)
	{
...
//////////////////////////////////////////////////////////////////////
void g4hrsUsageManager::ProcessReservedOption(branch *br)
{
	...
	if( (br->key1.compare("INPUTFILE")==0 || br->key1.compare("inputfile")==0) )
	{
		...
		{	
			...
			//calling g4hrsUsageManager::Getparameter(string name, T &para);
...
bool g4hrsUsageManager::GetArgument(const std::string name,int &argument)
{
	bool found=false;
...
bool g4hrsUsageManager::GetArgument(const std::string name,size_t &argument)
{
	int arg;
...
bool g4hrsUsageManager::GetArgument(const std::string name,float &argument)
{
	bool found=false;
...
bool g4hrsUsageManager::GetArgument(const std::string name,double &argument)
{
	float arg;
	bool found=GetArgument(name,arg);
	argument=(double)arg;
...
bool g4hrsUsageManager::GetArgument(const std::string name,std::string &argument)
{
	bool found=false;
...
bool g4hrsUsageManager::GetArgument(const std::string name,char* argument)
{
	string arg;
	bool found=GetArgument(name,arg);
	if(found)
...
std::string g4hrsUsageManager::GetArgument(std::string name)
{	
	bool found=false;
	std::string argument("");
	for(size_t i=0;i<VBranch.size();i++)
...
std::string g4hrsUsageManager::GetArgument(const char *name)
{ 	
...
bool g4hrsUsageManager::CheckPath(const char *filename)
{
	string file(filename);
	return CheckPath(file);
}

bool g4hrsUsageManager::CheckPath(string &val)
{
...
bool g4hrsUsageManager::ReadFile(const char *filename)
{
...
bool g4hrsUsageManager::ReadFile_c(const char *filename)
{
	FILE *ini;
...
//using the branch vector to find the variables
std::string g4hrsUsageManager::GetParameter(std::string name)
{	
	bool found=false;
...
std::string g4hrsUsageManager::GetParameter(const char *name)
{ 	
...
/*
bool g4hrsUsageManager::GetParameter(const std::string name,size_t &parameter)
{
...
bool g4hrsUsageManager::GetParameter(const std::string name,int    &parameter)
{
std::string para=GetParameter(name);
bool found=(para.length()==0)?false:true;
...
bool g4hrsUsageManager::GetParameter(const std::string name,float  &parameter)
{
std::string para=GetParameter(name);
bool found=(para.length()==0)?false:true;
...
bool g4hrsUsageManager::GetParameter(const std::string name,double &parameter)
{
std::string para=GetParameter(name);
bool found=(para.length()==0)?false:true;
...
*/
bool g4hrsUsageManager::GetParameter(const std::string name,std::string &parameter)
{
	parameter=GetParameter(name);
	bool found=(parameter.length()==0)?false:true;
	return found;
}
bool g4hrsUsageManager::GetParameter(const std::string name,char *parameter)
{
	std::string para=GetParameter(name);
	bool found=(para.length()==0)?false:true;
...
bool g4hrsUsageManager::SetParameter(std::string name,std::string value)
{
	bool found=false;
	map<string,string>::iterator it;
...
bool g4hrsUsageManager::SetParameter(std::string name,const char* value)
{
	std::string theStr(value);
	return SetParameter(name,theStr);
}

bool g4hrsUsageManager::SetParameter(std::string name,char value[])
{
	std::string theStr(value);
	return SetParameter(name,theStr);
}
...
bool g4hrsUsageManager::SetArgument(std::string name,std::string value)
{
	bool found=false;
	map<string,string>::iterator it;
...
bool g4hrsUsageManager::SetArgument(std::string name,const char* value)
{
	std::string theStr(value);
	return SetArgument(name,theStr);
}

bool g4hrsUsageManager::SetArgument(std::string name,char value[])
{
	std::string theStr(value);
	return SetArgument(name,theStr);
}
